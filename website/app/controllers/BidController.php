...
use Phalcon\Tag;
use EstadatSAS\Models\Bid2;
use EstadatSAS\Models\Users;
use EstadatSAS\Models\Paises;

use EstadatSAS\Forms\PgForm;
use EstadatSAS\Forms\DrhForm;
use EstadatSAS\Forms\ObjetivosForm;
use EstadatSAS\Forms\InreForm;
use EstadatSAS\Forms\SalariosForm;
use EstadatSAS\Forms\ComunicacionForm;
use EstadatSAS\Forms\TrabajoForm;
use EstadatSAS\Forms\EstiloForm;
use EstadatSAS\Forms\ConfianzaForm;
use EstadatSAS\Forms\SatisfaccionForm;
use EstadatSAS\Forms\ActividadesForm;
use EstadatSAS\Forms\SugerenciasForm;
use EstadatSAS\Forms\PercepcionForm;
use EstadatSAS\Forms\ServiciosForm;
use EstadatSAS\Forms\CondicionesForm;
use EstadatSAS\Forms\EstacionForm;
use EstadatSAS\Forms\RecepcionForm;
use EstadatSAS\Forms\SistemasForm;
use EstadatSAS\Forms\TransporteForm;
use EstadatSAS\Forms\AsistenteForm;
use EstadatSAS\Forms\ObservacionesForm;


class BidController extends ControllerBase
{
  public function initialize()
  {
    $this->view->setTemplateBefore('private');
    $cntBid     = count( Users::find("registro = 'Y' AND paisId < 16") );

    $Belice   = Paises::findFirstById(1);
    $Costa_Rica   = Paises::findFirstById(2);
    $El_Salvador   = Paises::findFirstById(3);
    $Guatemala   = Paises::findFirstById(4);
    $Honduras   = Paises::findFirstById(5);
    $Mexico   = Paises::findFirstById(8);
    $Nicaragua   = Paises::findFirstById(9);
    $Panama   = Paises::findFirstById(10);
    $Republica_Dominicana   = Paises::findFirstById(11);

    $this->view->paisesTotal  = $Belice->total + $Costa_Rica->total + $El_Salvador->total + $Guatemala->total + $Honduras->total + $Mexico->total + $Nicaragua->total + $Panama->total + $Republica_Dominicana->total;
  ...
  public function finalizardatosAction()
  {
      ...
      if($this->request->isAjax() == true)
      {
        ...
        if ($auth) {
          ...
          if (!$registro->save()) {
            ...
            $this->response->setJsonContent(array('res' => "Termino"));
